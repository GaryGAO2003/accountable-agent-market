...
退款和拦截都是**按事件**的：钱回来了、动作被停了——但同一个卖家下一轮照样可以中标、照样浪费
一轮。信誉层补上跨轮记忆：每轮的终局结果更新卖家分数（**settled +2**，**refunded / blocked /
验证失败 −3**——一次欺诈盖过一次好交付），分数触及冻结线的卖家被**永久冻结**：买家在授标前
直接丢弃它的出价。L1 拿回钱、L2 拦下一次动作、**L3 终止这段关系**——一次作恶，rogue 就再也
中不了标，连浪费轮次的机会都没有。

每次信誉变动都广播进 CoralOS 线程（`REPUTATION seller=… score=… tier=…`），并默认**通过 SPL
Memo 交易锚定上链**——信誉轨迹有真实的 Explorer 链接。诚实定位：memo 是买家签名的链上*日志*；
冻结本身是买家侧策略。完整的信誉*状态机*（自有程序、按卖家的 PDA、保证金随信誉缩放）是路线图
的下一级。
...
- **记忆层——信誉**（`packages/agent-runtime/src/market/reputation.ts`）：按卖家累积计分
  （settled +2 / refunded·blocked·验证失败 −3）、三档信誉（trusted/neutral/flagged）、市场层
  冻结——买家在授标前丢弃 flagged 卖家的出价，一次作恶即终止关系。信誉变动以 `REPUTATION`
  线程消息广播，并默认经 **SPL Memo** 锚定上链（`REP_MEMO=1`）；面板显示各卖家信誉档、
  出价行的 "frozen out" 徽章、以及 memo 轨迹的 Explorer 链接。
...
仲裁 agent → **自部署链上仲裁 + slashing**（第 3 条发现就是论据）→ 信誉 **PDA + 保证金随信誉
缩放**（链下账本 + Memo 轨迹已落地；链上状态机是下一步）→ 完整面板时间线。
